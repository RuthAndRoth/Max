// max_applier.lsl - SS Combo skin applier
...
integer can_haz_notecard(string name) {
    ...
    while (count--) {
        if (llGetInventoryName(INVENTORY_NOTECARD, count) == name) {
            log("max_applier: Found notecard: " + name);
            return TRUE;
        }
    }
    llOwnerSay("Notecard " + name + " not found, no textures will be loaded");
    return FALSE;
}
...
integer can_haz_script(string name) {
    ...
    while (count--) {
        if (llGetInventoryName(INVENTORY_SCRIPT, count) == name) {
            log("max_applier: Found script: " + name);
            return TRUE;
        }
    }
...
load_notecard(string name) {
    ...
    log("max_applier: Reading notecard: " + notecard_name);
...
reset() {
    ...
    log("max_applier: Free memory " + (string)llGetFreeMemory() + "  Limit: " + (string)MEM_LIMIT);
}
...
default {
    ...
    dataserver(key query_id, string data) {
        if (query_id == notecard_qid) {
            ...
            if (data == EOF) {
                // Do end work here
                reading_notecard = FALSE;
                llOwnerSay("max_applier: Finished reading notecard " + notecard_name);
                llOwnerSay("max_applier: Free memory " + (string)llGetFreeMemory() + "  Limit: " + (string)MEM_LIMIT);
                send_skin_thumbnails();
                send_eye_thumbnails();
                send_nail_colors();
            }
    ...
    link_message(integer sender_number, integer number, string message, key id) {
        if (number == LINK_OMEGA) {
            ...
            log("max_applier: command: " + command);
            if (command == "apply") {
        ...
        if (number == LINK_RUTH_APP) {
            ...
            if (command == "STATUS") {
                ...
                channel = get_channel(llList2Integer(cmdargs, 1));
            }
